...
有許多編號的大理石，按編號由小到大排好，接著回答一連串的問題：某個編號的大理石出現在第幾個位置？
- **輸入:** 多組測試案例，每組開頭為大理石數量 $N$ 與詢問次數 $Q$。接著是 $N$ 顆大理石的編號，以及 $Q$ 個詢問編號。當 $N=0, Q=0$ 時結束
- **預期輸出:** 若找到大理石，輸出 x found at y（$y$ 為從 1 開始的索引）；若未找到，輸出 x not found
- **主要規則或限制**
  1. 必須先對大理石進行升序排序
  2. 若有多顆相同編號的大理石，必須回傳第一個出現的位置
- **核心任務:** 實作排序後的高效搜尋
